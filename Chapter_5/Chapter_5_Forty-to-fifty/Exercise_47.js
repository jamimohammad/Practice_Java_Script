...
let user = prompt("Enter the first 12 digits of an ISBN-13");

if (user.length !== 12) {
  console.log(user + " is an invalid input");
} else {
  let sum = 0;

  for (let i = 0; i < 12; i++) {
    let digit = Number(user.charAt(i));
  ...
  let d13 = 10 - remainder;

  if (d13 === 10) {
    d13 = 0;
  }

  console.log("The ISBN-13 number is " + user + d13);
}
